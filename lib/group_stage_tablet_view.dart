...
class _GroupStageTabletViewState extends State<GroupStageTabletView> with TickerProviderStateMixin {
          ...
          actions: [
            ...
            if (groups.isNotEmpty) ...[
              ...
              IconButton(
                ...
                tooltip: _stickyPanelVisible ? '진행 현황 패널 숨기기' : '진행 현황 패널 보기',
                        ...
                        slivers: [
                          SliverPersistentHeader(
                            ...
                            delegate: _StickyHeaderDelegate(
                              height: 60,
                              selectedIndex: _currentEventIdx,
                              child: Material(elevation: 2, color: Colors.white, child: _buildEventTabBar()),
                            ),
                          ),
                        ...
                        Positioned(
                          ...
                          child: LayoutBuilder(
                            builder: (context, constraints) => SizedBox(
                              ...
                              child: _buildRightStickyProgressPanel(groups),
                            ),
                          ),
                        ),
                    ],
  ...
  Widget _buildEventTabBar() => Container(height: 60, color: Colors.white, child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 10), itemCount: widget.allEvents.length, itemBuilder: (context, index) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 10), child: ChoiceChip(label: Text(widget.allEvents[index].name), selected: isSelected, selectedColor: const Color(0xFF1A535C), labelStyle: TextStyle(color: isSelected ? Colors.white : Colors.black, fontWeight: FontWeight.bold), onSelected: (v) {
      if (v) { if (_waitingPlayers.isNotEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('대기 명단 선수를 먼저 배정해주세요.'))); return; } setState(() { _currentEventIdx = index; _refreshRankings(); _findWaitingPlayers(); }); }
    }));
  }));
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Slider(value: _stickyPanelOpacity, onChanged: (v) => setState(() => _stickyPanelOpacity = v), activeColor: Colors.white, inactiveColor: Colors.white24)),
                    ]),
                  ),
                ],
              ),
  ...
  Widget _buildProgressTab(List<Group> groups) {
    return SingleChildScrollView(
      ...
      child: Wrap(
        spacing: 4,
        runSpacing: 4,
        children: List.generate(groups.length, (gIdx) => SizedBox(
          width: 42,
          height: 42,
          child: _buildGroupProgressBox(groups[gIdx], gIdx + 1, onTap: () => _scrollToGroupCard(gIdx)),
        )),
      ),
    );
  }
      ...
      for (int i = 0; i < _currentEvent.groups!.length; i++) {
        for (var p in _currentEvent.groups![i].players) {
          if (query.isEmpty || p.name.toLowerCase().contains(query) || p.affiliation.toLowerCase().contains(query)) {
            allPlayersInfo.add({'player': p, 'groupIdx': i, 'groupName': _currentEvent.groups![i].name});
          }
        }
      }
    }
      ...
      children: [
        Padding(
          ...
          child: TextField(
            ...
            onChanged: (v) => setState(() => _rightPanelSearchQuery = v),
          ),
        ),
        Expanded(
          child: ListView.separated(
            padding: EdgeInsets.zero,
            itemCount: allPlayersInfo.length,
            separatorBuilder: (ctx, i) => const Divider(height: 1),
            itemBuilder: (ctx, idx) {
              final info = allPlayersInfo[idx];
              ...
              return ListTile(
                ...
                title: Text(_isTeamMatch ? p.affiliation : p.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
                subtitle: Text(gName, style: const TextStyle(fontSize: 11, color: Colors.blueGrey)),
                onTap: () {
                  if (gIdx >= 0) {
                    _scrollToGroupCard(gIdx);
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('대기 명단에 있는 선수입니다.'), duration: Duration(seconds: 1)));
                  }
                },
              );
            },
  ...
  void _scrollToGroupCard(int groupIndex) {
    if (groupIndex < 0 || groupIndex >= _groupCardKeys.length) return;
    final key = _groupCardKeys[groupIndex];
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: SizedBox(width: double.maxFinite, child: Column(mainAxisSize: MainAxisSize.min, children: [
        ConstrainedBox(constraints: BoxConstraints(maxHeight: MediaQuery.of(context).size.height * 0.4), child: ListView.builder(shrinkWrap: true, itemCount: _currentEvent.groups?.length ?? 0, itemBuilder: (ctx, idx) {
          ...
          return ListTile(title: Text(g.name), subtitle: Text('${g.players.length}명 배정됨'), trailing: Icon(canAdd ? Icons.add_circle_outline : Icons.warning, color: canAdd ? Colors.green : Colors.orange), onTap: () {
            setState(() { g.players.add(p); _waitingPlayers.remove(p); TournamentLogic.syncGroupMatches(g, idx + 1); _refreshRankings(); });
            widget.onDataChanged(); Navigator.pop(ctx);
          });
        })),
        const Divider(),
        ListTile(leading: const Icon(Icons.add_box_outlined, color: Colors.blue), title: const Text('새로운 조 생성 및 배정', style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)), onTap: () {
          Navigator.pop(ctx); _addNewGroup(); final lastIdx = _currentEvent.groups!.length - 1; final newGroup = _currentEvent.groups![lastIdx];
          setState(() { newGroup.players.add(p); _waitingPlayers.remove(p); TournamentLogic.syncGroupMatches(newGroup, lastIdx + 1); _refreshRankings(); });
          widget.onDataChanged();
        }),
      ])),
  ...
  Widget _buildGroupCard(Group group, int groupIdx) {
    ...
    final maxGroupSize = _currentEvent.settings.groupSize;
    final isUnder = group.players.length < maxGroupSize;
    return Card(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), elevation: isUnder ? 4 : 1,
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 20),
          decoration: BoxDecoration(color: isUnder ? Colors.red.shade50 : const Color(0xFFF8F9FA), borderRadius: const BorderRadius.vertical(top: Radius.circular(16))),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Row(children: [
              Text('${group.players.length} / $maxGroupSize명', style: TextStyle(color: isUnder ? Colors.red : Colors.grey, fontSize: 14, fontWeight: isUnder ? FontWeight.bold : FontWeight.normal)),
              const SizedBox(width: 10),
              IconButton(icon: const Icon(Icons.print, color: Colors.blue, size: 24), onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => MatchSheetPage(tournamentTitle: '${widget.tournamentBaseTitle} - ${_currentEvent.name}', groups: [group], knockoutRounds: _currentEvent.knockoutRounds)));
  ...
  void _showPlayerActionDialog(Player player, int currentGroupIdx) {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(title: Text('${_isTeamMatch ? player.affiliation : player.name} 관리'), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ListTile(leading: Icon(Icons.logout, color: hasRecord ? Colors.grey : Colors.orange), title: Text('대기 명단으로 보내기', style: TextStyle(color: hasRecord ? Colors.grey : Colors.black)), onTap: hasRecord ? null : () {
        setState(() { group.players.remove(player); _waitingPlayers.insert(0, player); TournamentLogic.syncGroupMatches(group, currentGroupIdx + 1); _refreshRankings(); });
        widget.onDataChanged(); Navigator.pop(ctx);
      }),
      const Divider(),
      if (_waitingPlayers.isNotEmpty) ...[
        ...
        ..._waitingPlayers.map((waitingP) => ListTile(dense: true, visualDensity: VisualDensity.compact, leading: Icon(Icons.swap_horiz, color: hasRecord ? Colors.grey : Colors.blue), title: Text(_isTeamMatch ? waitingP.affiliation : waitingP.name, style: TextStyle(color: hasRecord ? Colors.grey : Colors.black)), onTap: hasRecord ? null : () {
          setState(() { int idx = group.players.indexOf(player); if (idx != -1) { group.players[idx] = waitingP; _waitingPlayers.remove(waitingP); _waitingPlayers.insert(0, player); TournamentLogic.syncGroupMatches(group, currentGroupIdx + 1); _refreshRankings(); } });
          widget.onDataChanged(); Navigator.pop(ctx);
        })),
        const Divider(),
        if (canAdd) ...[
          const Padding(padding: EdgeInsets.symmetric(vertical: 8.0), child: Text('대기 선수 충원하기', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green))),
          ..._waitingPlayers.map((waitingP) => ListTile(dense: true, visualDensity: VisualDensity.compact, leading: const Icon(Icons.person_add, color: Colors.green), title: Text(_isTeamMatch ? waitingP.affiliation : waitingP.name), onTap: () {
            setState(() { group.players.add(waitingP); _waitingPlayers.remove(waitingP); TournamentLogic.syncGroupMatches(group, currentGroupIdx + 1); _refreshRankings(); });
            widget.onDataChanged(); Navigator.pop(ctx);
          })),
        ],
      ]
