...
class _SetupMobileViewState extends State<SetupMobileView> with SetupLogicMixin {
  ...
  void _goToGroupStage() { if (currentEvent == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('진행 종목을 선택해주세요.'))); return; } currentEvent!.groups ??= TournamentLogic.generateGroups(currentEvent!.players, currentEvent!.settings); saveData(); Navigator.push(context, MaterialPageRoute(builder: (context) => GroupStagePage(tournamentBaseTitle: titleController.text, allEvents: events, initialEventIdx: selectedEventIdx, onDataChanged: saveData))).then((result) { if (result is int) setState(() => selectedEventIdx = result); }); }
  void _goToKnockout() { if (currentEvent == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('진행 종목을 선택해주세요.'))); return; } if (currentEvent!.knockoutRounds == null || currentEvent!.knockoutRounds!.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('본선 대진이 없습니다. 예선전에서 본선 진출 생성 후 이동해주세요.'))); return; } Navigator.push(context, MaterialPageRoute(builder: (context) => KnockoutPage(tournamentTitle: '${titleController.text} - ${currentEvent!.name}', rounds: currentEvent!.knockoutRounds!, onDataChanged: saveData, events: events))); }
  void _startTournament() async { if (currentEvent!.groups == null) currentEvent!.groups = TournamentLogic.generateGroups(currentEvent!.players, currentEvent!.settings); saveData(); final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => GroupStagePage(tournamentBaseTitle: titleController.text, allEvents: events, initialEventIdx: selectedEventIdx, onDataChanged: saveData))); if (result is int) setState(() => selectedEventIdx = result); }
