...
class _GroupStageMobileViewState extends State<GroupStageMobileView> {
  ...
  void _generateKnockout() async {
    ...
    final bool isSkipMode = _currentEvent.settings.skipGroupStage;
    if (isSkipMode) { qualified = List.from(_currentEvent.players); }
    else {
  ...
  Widget _buildRankingTable(List<Player> rankings, Group group, int groupIdx) {
    ...
    return Padding(
      ...
      child: Table(
        ...
        columnWidths: const { 0: FlexColumnWidth(0.5), 1: FlexColumnWidth(2.2), 2: FlexColumnWidth(0.5), 3: FlexColumnWidth(0.5), 4: FlexColumnWidth(0.5), 5: FlexColumnWidth(0.6), 6: FlexColumnWidth(0.4) },
        ...
        children: [
          ...
          ...rankings.asMap().entries.map((e) {
            ...
            return TableRow(children: [
              ...
              TableCell(child: Row(mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, children: [GestureDetector(onTap: index == 0 ? null : () => _moveRank(groupIdx, index, -1), child: Icon(Icons.arrow_upward, size: 16, color: index == 0 ? Colors.grey.shade300 : Colors.blue)), const SizedBox(width: 2), GestureDetector(onTap: index == rankings.length - 1 ? null : () => _moveRank(groupIdx, index, 1), child: Icon(Icons.arrow_downward, size: 16, color: index == rankings.length - 1 ? Colors.grey.shade300 : Colors.red))])),
            ]);
          }),
        ],
      ),
  ...
  Widget _buildMatchTile(Match m, Group group) {
    if (!_isTeamMatch) {
      return ListTile(dense: true, title: Row(children: [Expanded(child: Text(m.player1?.name ?? 'BYE', textAlign: TextAlign.right, style: const TextStyle(fontSize: 12))), Container(margin: const EdgeInsets.symmetric(horizontal: 10), child: Text('${m.score1} : ${m.score2}', style: const TextStyle(fontWeight: FontWeight.bold))), Expanded(child: Text(m.player2?.name ?? 'BYE', textAlign: TextAlign.left, style: const TextStyle(fontSize: 12)))]), subtitle: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Expanded(child: Text(m.player1?.affiliation ?? '', textAlign: TextAlign.right, style: const TextStyle(fontSize: 10, color: Colors.grey))), const SizedBox(width: 40), Expanded(child: Text(m.player2?.affiliation ?? '', textAlign: TextAlign.left, style: const TextStyle(fontSize: 10, color: Colors.grey)))]), onTap: () => _maybeShowScoreDialog(m, group));
    }
    return InkWell(onTap: () => _maybeShowScoreDialog(m, group), child: Padding(padding: const EdgeInsets.symmetric(vertical: 12.0, horizontal: 16.0), child: Column(children: [Row(children: [Expanded(child: Text(m.player1?.affiliation ?? 'BYE', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF1A535C)))), const SizedBox(width: 60), Expanded(child: Text(m.player2?.affiliation ?? 'BYE', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF1A535C))))]), const SizedBox(height: 6), Row(crossAxisAlignment: CrossAxisAlignment.center, children: [Expanded(child: Text(_formatTeamMembers(m.player1?.name ?? ''), textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, color: Colors.black87, height: 1.3))), Container(width: 60, alignment: Alignment.center, child: Text(m.status == MatchStatus.withdrawal ? '기권' : '${m.score1} : ${m.score2}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.redAccent))), Expanded(child: Text(_formatTeamMembers(m.player2?.name ?? ''), textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, color: Colors.black87, height: 1.3)))])])));
  }
