...
class KnockoutMobileView extends StatefulWidget {
  final String tournamentTitle;
  final List<Round> rounds;
  final VoidCallback onDataChanged;
  final List<TournamentEvent> events;

  const KnockoutMobileView({
    super.key,
  ...
  @override
  State<KnockoutMobileView> createState() => _KnockoutMobileViewState();
}

class _KnockoutMobileViewState extends State<KnockoutMobileView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: false,
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('예선전', style: TextStyle(color: Colors.white, fontSize: 13))),
          TextButton(onPressed: () { Navigator.pop(context); Navigator.pop(context); }, child: const Text('메인', style: TextStyle(color: Colors.white, fontSize: 13))),
          ...
          IconButton(
            icon: Icon(_isSelectionExpanded ? Icons.expand_less : Icons.apps, color: Colors.blue),
          ...
          ? PreferredSize(
              ...
              child: Container(
                ...
                child: otherActiveEvents.isEmpty 
                  ? const Center(child: Text('다른 진행 중인 종목이 없습니다.', style: TextStyle(fontSize: 12, color: Colors.grey)))
                  : ListView.builder(
  ...
  Widget _playerRow(Player? p, int s, bool isW, bool withdrawn) {
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(p?.affiliation ?? (p == null ? 'BYE' : 'TBD'), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isW ? const Color(0xFF1A535C) : Colors.black), overflow: TextOverflow.ellipsis),
                    if (p != null) Text(_formatTeamMembers(p.name), style: TextStyle(fontSize: 12, color: isW ? const Color(0xFF1A535C) : Colors.black87, height: 1.25, fontWeight: FontWeight.w600), maxLines: 3, overflow: TextOverflow.ellipsis),
                  ],
                ),
              ),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                decoration: BoxDecoration(color: isW ? const Color(0xFF1A535C) : Colors.grey.shade200, borderRadius: BorderRadius.circular(4)),
                child: Text(withdrawn ? '기권' : '$s', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: isW ? Colors.white : Colors.black)),
              ),
            ],
          ),
        ),
    ...
    return Expanded(
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(p?.name ?? (p == null ? 'BYE' : 'TBD'), style: TextStyle(fontWeight: isW ? FontWeight.bold : FontWeight.normal, fontSize: 13), overflow: TextOverflow.ellipsis), if (p != null) Text(p.affiliation, style: const TextStyle(fontSize: 11, color: Colors.grey), overflow: TextOverflow.ellipsis)])),
      ...
      builder: (context) => StatefulBuilder(
          builder: (context, setS) => Container(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_counter((v) => setS(() => s1 = v), s1), const Text(':', style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold)), _counter((v) => setS(() => s2 = v), s2)]),
                  const SizedBox(height: 20),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                    TextButton.icon(onPressed: () => setS(() { s1 = -1; s2 = 0; }), icon: const Icon(Icons.flag, color: Colors.red), label: const Text('P1 기권')),
                    TextButton.icon(onPressed: () => setS(() { s1 = 0; s2 = -1; }), icon: const Icon(Icons.flag, color: Colors.red), label: const Text('P2 기권')),
                  ]),
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: () {
  ...
  Widget _playerPopupInfo(Player? p, {required TextAlign textAlign}) => Column(
          ...
          children: [
            Text(_isTeamMatch ? (p?.affiliation ?? "TBD") : (p?.name ?? "TBD"), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis),
            Text(_isTeamMatch ? (p?.name ?? "") : (p?.affiliation ?? ""), style: const TextStyle(fontSize: 14, color: Colors.orange), overflow: TextOverflow.ellipsis),
          ]);

  Widget _counter(Function(int) onU, int v) => Column(children: [
        IconButton(onPressed: () => onU(v + 1), icon: const Icon(Icons.add_circle, color: Color(0xFF4ECDC4), size: 48)),
        Text(v == -1 ? '기권' : '$v', style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Color(0xFF1A535C))),
        IconButton(onPressed: () => onU(v > 0 ? v - 1 : 0), icon: const Icon(Icons.remove_circle, color: Colors.redAccent, size: 48)),
      ]);
}
