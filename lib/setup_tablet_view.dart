...
const List<String> _tierOptions = [
  '남 1부', '남 2부', '남 3부', '남 4부', '남 5부', '남 6부',
  '여 1부', '여 2부', '여 3부', '여 4부', '여 5부', '여 6부',
  '혼합 1부', '혼합 2부', '혼합 3부', '혼합 4부', '혼합 5부',
];
...
class _SetupTabletViewState extends State<SetupTabletView> with SetupLogicMixin<SetupTabletView>, SingleTickerProviderStateMixin {
  ...
  String? _editingTeamId; // 현재 수정 중인 팀 ID (null이면 새로 추가)
  ...
  void initState() {
    ...
    _playerTabController.addListener(() {
      if (!mounted) return;
      // 선수 선택 탭 진입 시: 패널 확장 플래그 초기화
      if (_playerTabController.index == 1) {
        setState(() {
          _leftPanelTapped = false;
        });
      }
      // 참가 현황 탭 진입 시: 약간의 딜레이 후 명단을 한 번 더 갱신해서
      // 선수 선택 탭에서 추가된 선수가 확실히 반영되도록 함
      if (_playerTabController.index == 0) {
        Future.delayed(const Duration(milliseconds: 200), () {
          if (!mounted) return;
          // 여전히 참가 현황 탭이면 강제로 한 번 더 빌드
          if (_playerTabController.index == 0) {
            setState(() {});
          }
        });
  ...
  bool _isPlayerInRegisteredTeams(String playerName) {
    ...
    for (var player in currentEvent!.players) {
      // 단체전의 경우 player.name에 "선수1, 선수2, 선수3" 형태로 저장됨
      final teamMembers = player.name.split(', ').map((s) => s.trim()).toList();
      if (teamMembers.contains(playerName.trim())) {
        return true;
      }
    }
    return false;
  ...
  // 선수가 이미 등록되어 있는지 확인 (입력칸 또는 참가명단)
  ...
  void _loadTeamToInputFields(Player teamPlayer) {
    ...
    // 현재 수정 중인 팀 ID 저장 (수정 저장 시 사용)
    _editingTeamId = teamPlayer.id;
    
    setState(() {});
  }

  // 단체전일 때 선수 선택 시 입력칸에 자동으로 채우기
  void _addPlayerToTeamInput(MasterPlayer p) {
    ...
    // 수정 모드일 때는 현재 편집 중인 팀의 선수는 중복 체크에서 제외
    bool isInCurrentEditingTeam = false;
    if (_editingTeamId != null) {
      final existingTeam = currentEvent!.players.firstWhere(
        (e) => e.id == _editingTeamId,
        orElse: () => Player(id: '', name: '', affiliation: '')
      );
    ...
    // 이미 입력칸 또는 참가명단에 등록되어 있는지 확인 (현재 편집 중인 팀 제외)
    if (!isInCurrentEditingTeam && _isPlayerAlreadyRegistered(playerName)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('이미 등록된 선수입니다.'), duration: Duration(seconds: 1))
      );
      return; // 이미 등록되어 있으면 무시
    }
    
    // 빈칸을 찾아서 순차적으로 입력
    int emptyIndex = -1;
    for (int i = 0; i < totalMembers && i < _teamMemberControllers.length; i++) {
      if (_teamMemberControllers[i].text.trim().isEmpty) {
        emptyIndex = i;
        break;
      }
    }
    ...
    // 빈칸이 없으면 마지막 칸에 입력
    if (emptyIndex == -1 && _teamMemberControllers.isNotEmpty) {
      emptyIndex = _teamMemberControllers.length - 1;
    }
  ...
  void _onNameChanged() async {
    ...
    if (mounted) {
      setState(() { _searchedMasterPlayers = results; _showAutoComplete = results.isNotEmpty; });
    }
  }

  void _selectMasterPlayer(MasterPlayer p) {
    setState(() {
      _nameController.text = "${p.name}(${p.tier})";
      _affController.text = p.affiliation;
  ...
  Future<void> _handleDeleteMaster(MasterPlayer p) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('선수 정보 삭제'),
        content: Text('[${p.name}] 선수를 DB에서 영구 삭제하시겠습니까?'),
        actions: [
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: numEdit, decoration: const InputDecoration(labelText: '번호')),
          TextField(controller: cityEdit, decoration: const InputDecoration(labelText: '지역')),
          TextField(controller: affEdit, decoration: const InputDecoration(labelText: '동호회')),
          TextField(controller: nameEdit, decoration: const InputDecoration(labelText: '이름')),
          TextField(controller: genderEdit, decoration: const InputDecoration(labelText: '성별')),
          TextField(controller: tierEdit, decoration: const InputDecoration(labelText: '부수')),
          TextField(controller: pointsEdit, decoration: const InputDecoration(labelText: '누적점수')),
        ])),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소')),
          TextButton(onPressed: () async {
            final updated = MasterPlayer(
              id: p.id,
              playerNumber: numEdit.text.trim(),
              city: cityEdit.text.trim(),
              affiliation: affEdit.text.trim(),
              name: nameEdit.text.trim(),
              gender: genderEdit.text.trim(),
              tier: tierEdit.text.trim(),
              points: pointsEdit.text.trim(),
            );
            await dbService.updatePlayer(p.id!, updated);
            _refreshMasterList();
            Navigator.pop(ctx);
          }, child: const Text('수정')),
        ],
  ...
  void _goToKnockout() {
    ...
    if (currentEvent!.knockoutRounds == null || currentEvent!.knockoutRounds!.isEmpty) {
        ...
        Expanded(child: _isDbManagementMode ? _buildDbManagementArea() : LayoutBuilder(builder: (context, constraints) {
          ...
          return Row(children: [
            GestureDetector(
              ...
              onTap: () => setState(() => _leftPanelTapped = true),
              child: SizedBox(key: _leftPanelKey, width: leftWidth, child: _buildMainInputArea()),
            ),
            ...
            onDeleteEvent: (eventId) {
              ...
              if (idx >= 0) {
                setState(() {
                  events.removeAt(idx);
                  if (selectedEventIdx >= events.length) {
                    selectedEventIdx = events.length > 0 ? 0 : 0;
                  }
                });
                saveData();
              }
  ...
  void _showDuplicatesList() async {
    ...
    if (!mounted) return;
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: const Text('중복 데이터 목록 (저장되지 않음)'),
      content: SizedBox(width: 500, height: 400, child: ListView.separated(itemCount: dups.length, separatorBuilder: (ctx, i) => const Divider(), itemBuilder: (ctx, i) => ListTile(title: Text('${dups[i].name} (${dups[i].tier})'), subtitle: Text('${dups[i].city} - ${dups[i].affiliation}'), dense: true))),
              ...
              child: GestureDetector(
                ...
                child: ChoiceChip(
                  ...
                  onSelected: (v) {
                    if (v) {
                      setState(() {
                        selectedEventIdx = e.key;
                        // 종목 선택 시 단체전 인원수에 맞게 컨트롤러 업데이트
                        if (currentEvent != null && currentEvent!.teamSize > 1) {
                          _updateTeamMemberControllers(currentEvent!.teamSize);
                        }
                      });
  ...
  void _secureSettingUpdate(VoidCallback updateAction, {bool isPlayerAction = false}) {
    if (isPlayerAction && currentEvent!.knockoutRounds != null && currentEvent!.knockoutRounds!.isNotEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('본선 토너먼트가 시작되어 선수를 추가/삭제할 수 없습니다.'), backgroundColor: Colors.redAccent)); return; }
    if (currentEvent!.groups != null) {
      final pwController = TextEditingController();
      showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text('관리자 확인'), content: Column(mainAxisSize: MainAxisSize.min, children: [const Text('관리자의 허락없이 수정할 수 없습니다.', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)), const SizedBox(height: 16), TextField(controller: pwController, obscureText: true, decoration: const InputDecoration(labelText: '비밀번호 입력', border: OutlineInputBorder()))],), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소')), TextButton(onPressed: () { if (pwController.text == '1234') { setState(updateAction); Navigator.pop(ctx); } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('비밀번호가 틀렸습니다.'))); } }, child: const Text('확인'))],));
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                ...
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Checkbox(
                    ...
                    value: showTierFilter,
                    onChanged: (v) { _secureSettingUpdate(() { currentEvent!.settings.showTierFilter = v!; currentEvent!.groups = null; saveData(); }); },
                  ),
                ]),
              ]),
                  ...
                  return FilterChip(
                    ...
                    onSelected: (v) {
                      _secureSettingUpdate(() {
                        if (v) currentEvent!.settings.allowedTiers.add(tier);
                        else currentEvent!.settings.allowedTiers.remove(tier);
                        currentEvent!.settings.allowedTiers = List.from(currentEvent!.settings.allowedTiers);
  ...
  Widget _buildPlayerInputCard() {
    ...
    // 단체전인 경우 인원수에 따라 컨트롤러 업데이트 (반드시 먼저 호출)
    if (isTeam) {
      _updateTeamMemberControllers(currentEvent!.teamSize);
    }
    ...
    return Stack(clipBehavior: Clip.none, children: [
      Card(
        ...
        color: isKnockoutStarted ? Colors.grey.shade50 : Colors.white, 
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text(
                    isTeam ? '${currentEvent!.teamSize}인 단체전 선수 등록' : '개인전 선수 등록', 
                    style: Theme.of(context).textTheme.titleLarge?.copyWith(
                      fontWeight: FontWeight.bold, 
                      color: isKnockoutStarted ? Colors.grey : const Color(0xFF1A535C)
                    )
                  ), 
                  if (isKnockoutStarted) const Padding(
                    padding: EdgeInsets.only(left: 12), 
                    child: Text('(본선 시작으로 추가 불가)', style: TextStyle(color: Colors.red, fontSize: 13, fontWeight: FontWeight.bold))
                  )
                ]
              ), 
              const SizedBox(height: 24), 
              if (isTeam) ...[
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: '팀명 (클럽명)', border: OutlineInputBorder())
                ), 
                const SizedBox(height: 20), 
                // 단체전 인원수 + 예비 선수 1명만큼 입력칸 생성 (안전하게 체크)
                Wrap(
                  spacing: 16, 
                  runSpacing: 16, 
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                labelText: i < currentEvent!.teamSize ? '${i + 1}번 선수 이름(부수)' : '예비 선수 이름(부수)', 
                                border: const OutlineInputBorder()
                              ),
                              onChanged: (value) {
                                // 입력칸에서 선수가 삭제되면 선수 선택 탭 색상 업데이트
              ...
              const SizedBox(height: 32), 
              ElevatedButton.icon(
                onPressed: isKnockoutStarted ? null : _addPlayer, 
                icon: const Icon(Icons.person_add_alt_1), 
                label: const Text('참가 명단에 추가하기', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), 
      ...
      if (_showAutoComplete && !isTeam) Positioned(
        top: 145, 
        left: 32, 
        right: 32, 
        child: Material(
          ...
          child: Container(
            ...
            child: ListView.separated(
              ...
              itemCount: _searchedMasterPlayers.length, 
              separatorBuilder: (ctx, i) => const Divider(height: 1), 
              itemBuilder: (ctx, idx) {
                final p = _searchedMasterPlayers[idx]; 
                return ListTile(
                  leading: const Icon(Icons.person_search, color: Color(0xFF1A535C)), 
                  title: Text("${p.name} (${p.tier})", style: const TextStyle(fontWeight: FontWeight.bold)), 
                  subtitle: Text("${p.city} - ${p.affiliation}"), 
                  trailing: const Text('선택하기', style: TextStyle(color: Colors.blue, fontSize: 12)), 
                  onTap: () => _selectMasterPlayer(p)
                );
              }
            )
          )
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              const Text('참가 관리', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
                decoration: BoxDecoration(color: const Color(0xFFF0F4F8), borderRadius: BorderRadius.circular(20)),
                child: Text('총 ${currentEvent!.players.length}명', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1A535C), fontSize: 13)),
              ),
            ],
          ),
        ),
        ...
        TabBar(
          ...
          indicatorColor: const Color(0xFF1A535C),
          tabs: const [
            Tab(text: '참가 현황'),
            Tab(text: '선수 선택'),
          ],
        ...
        Expanded(
          child: TabBarView(
            controller: _playerTabController,
            children: [
              _buildParticipantTab(isPanelExpanded: isPanelExpanded),
              _buildPlayerSelectTab(isPanelExpanded: isPanelExpanded),
            ],
          ),
        ),
      ]),
  ...
  /// 참가 현황 탭: 기존 참가자 리스트 + 예선 대진표 시작 버튼 (탭 전환 시 명단 갱신)
  Widget _buildParticipantTab({required bool isPanelExpanded}) {
    final playerCount = currentEvent!.players.length;
    final isTeam = currentEvent!.teamSize > 1;
    final int columnCount = isPanelExpanded ? 4 : 3;

    return Column(
      key: ValueKey('participant_list_$playerCount'),
      children: [
        Expanded(
          child: playerCount == 0
              ? const Center(child: Text('참가 명단이 비어 있습니다.', style: TextStyle(color: Colors.grey)))
              : GridView.builder(
                  key: ValueKey('participant_grid_$playerCount'),
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 2),
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    crossAxisCount: columnCount,
                    childAspectRatio: isTeam ? 2.0 : 3.4, // 단체전일 때 높이 증가
                    crossAxisSpacing: 8,
                    mainAxisSpacing: 4,
                  ),
                  ...
                  itemBuilder: (ctx, index) {
                    ...
                    final displayIndex = currentEvent!.players.length - index;
                    final capturedIndex = index;
                    final capturedPlayer = p;
                            ...
                            children: [
                              ListTile(
                                ...
                                onTap: () {
                                  ...
                                  if (!mounted) return;
                                  _loadTeamToInputFields(capturedPlayer);
                                },
                              ),
                              ListTile(
                                ...
                                onTap: () {
                                  ...
                                  if (!mounted) return;
                                  _secureSettingUpdate(() {
                                    currentEvent!.players.removeWhere((e) => e.id == capturedPlayer.id);
                                    saveData();
                                  }, isPlayerAction: true);
                                },
                              ),
                    ...
                    if (isTeam) {
                      ...
                      return Card(
                        ...
                        child: InkWell(
                          onTap: () => _loadTeamToInputFields(p),
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // 선수 목록 표시 (한 줄에 2명씩, 최대 3줄)
                                ...List.generate(
                                  (teamMembers.length / 2).ceil(),
                                  (rowIndex) {
                                    // 최대 3줄까지만 표시
                                    if (rowIndex >= 3) return const SizedBox.shrink();
                                    final startIdx = rowIndex * 2;
                                    final endIdx = (startIdx + 2 < teamMembers.length) ? startIdx + 2 : teamMembers.length;
                                    return Padding(
                                      padding: const EdgeInsets.only(left: 30, bottom: 0.5),
                                      child: Row(
                                        children: [
                                          Expanded(
                                            child: Text(
        ...
        Padding(
          ...
          child: ElevatedButton(
            ...
            child: const Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text('예선 대진표 관리 시작', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                SizedBox(width: 12),
  ...
  /// 선수 선택 탭: 기초 선수 DB에서 지역/동호회/이름으로 검색 후 참가 명단에 추가. 부수설정이 있으면 체크한 부수만 표시.
  /// [isPanelExpanded] true면 4열, false면 3열(폭이 줄었을 때)로 표시하여 오버플로우 방지.
  Widget _buildPlayerSelectTab({required bool isPanelExpanded}) {
    final q = _playerSelectQuery.toLowerCase();
    final allowedTiers = currentEvent!.settings.allowedTiers;
    List<MasterPlayer> candidates = _allMasterPlayers.where((p) {
      if (q.isNotEmpty && !p.city.toLowerCase().contains(q) && !p.affiliation.toLowerCase().contains(q) && !p.name.toLowerCase().contains(q)) return false;
      // 부수설정에 1개 이상 체크되어 있으면 해당 부수만 표시
      if (allowedTiers.isNotEmpty && !allowedTiers.contains(p.tier)) return false;
      return true;
    }).toList();
    ...
    return Column(
      children: [
        Padding(
          ...
          child: TextField(
            decoration: const InputDecoration(
              labelText: '지역 / 동호회 / 이름 검색',
              prefixIcon: Icon(Icons.search),
              border: OutlineInputBorder(),
              isDense: true,
            ),
            onChanged: (v) => setState(() => _playerSelectQuery = v),
          ),
        ),
        if (isTeam)
          const Padding(
          ...
          child: candidates.isEmpty
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (ctx, idx) {
                    ...
                    final bool isKnockoutStarted = currentEvent!.knockoutRounds != null && currentEvent!.knockoutRounds!.isNotEmpty;
                    final String displayName = '${p.name} (${p.tier})';
                    
                    // 단체전일 때는 입력칸 또는 참가명단에 등록되어 있는지 확인
                    final bool isAlreadyRegistered = isTeam 
                        ? _isPlayerAlreadyRegistered(displayName)
                        : currentEvent!.players.any((e) => e.name == displayName);
                    
                    // 등록 상태에 따라 색상 결정 (등록되어 있으면 파란색)
                    final circleColor = isAlreadyRegistered ? Colors.blue : const Color(0xFF1A535C);
                    
                    // 선택 가능 여부: 본선이 시작되지 않았고, 아직 등록되지 않은 경우만
                    final bool canAdd = !isKnockoutStarted && !isAlreadyRegistered;
                          ...
                          children: [
                            GestureDetector(
                              onTap: canAdd
                                  ? () {
                                      if (isTeam) {
                                        // 단체전일 때는 입력칸에 자동으로 추가
                                        _addPlayerToTeamInput(p);
                                      } else {
                                        // 개인전일 때는 기존 로직대로 참가명단에 추가
                                        _secureSettingUpdate(() {
                                          currentEvent!.players.insert(
                                            0,
                                            Player(
                                              id: p.id != null ? p.id!.toString() : uuid.v4(),
                                              name: displayName,
                                              affiliation: p.affiliation.isNotEmpty ? p.affiliation : (p.city.isNotEmpty ? p.city : '개인'),
                                            ),
                                          );
                                          saveData();
                                        }, isPlayerAction: true);
                              ...
                              child: CircleAvatar(
                                ...
                                radius: 16,
                                child: Text(
                                  p.playerNumber.isNotEmpty ? p.playerNumber : '${idx + 1}',
                                  style: const TextStyle(color: Colors.white, fontSize: 10),
                                ),
                              ),
                            ),
                              ...
                              child: FittedBox(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('${p.city} · ${p.affiliation}', style: const TextStyle(fontSize: 12, color: Colors.black87), maxLines: 1, overflow: TextOverflow.ellipsis),
                                  ],
                                ),
                              ),
                            ),
  ...
  Widget _buildPlayerTile(int index) {
    final p = currentEvent!.players[index]; bool isTeam = currentEvent!.teamSize > 1; List<String> teamMembers = isTeam ? p.name.split(', ') : []; bool isKnockoutStarted = currentEvent!.knockoutRounds != null && currentEvent!.knockoutRounds!.isNotEmpty;
    return Container(
      decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12)),
      child: ListTile(
        contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), leading: CircleAvatar(backgroundColor: const Color(0xFF1A535C), radius: 18, child: Text('${currentEvent!.players.length - index}', style: const TextStyle(color: Colors.white, fontSize: 12))),
        title: Text(isTeam ? p.affiliation : p.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18), maxLines: 1, overflow: TextOverflow.ellipsis),
        subtitle: isTeam ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const SizedBox(height: 4), for (int i = 0; i < teamMembers.length; i += 2) Padding(padding: const EdgeInsets.only(bottom: 2), child: Row(children: [Expanded(child: Text(teamMembers[i], style: const TextStyle(fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis)), if (i + 1 < teamMembers.length) Expanded(child: Text(teamMembers[i+1], style: const TextStyle(fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis)), if (i + 1 >= teamMembers.length) const Expanded(child: SizedBox())]))]) : Text(p.affiliation, style: const TextStyle(fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis),
        trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(icon: Icon(Icons.edit_outlined, color: isKnockoutStarted ? Colors.grey : Colors.blue, size: 24), onPressed: () => _handleEditPlayer(index, p), tooltip: '수정'), IconButton(icon: Icon(Icons.delete_outline, color: isKnockoutStarted ? Colors.grey : Colors.redAccent, size: 24), onPressed: () { _secureSettingUpdate(() { currentEvent!.players.removeAt(index); saveData(); }, isPlayerAction: true); })],),
      ),
    );
  }
  ...
  void _handleEditPlayer(int index, Player player) {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(isTeam ? '팀 정보 수정' : '선수 정보 수정'),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: editAffController, decoration: InputDecoration(labelText: isTeam ? '팀명 (클럽명)' : '소속 클럽명')),
        const SizedBox(height: 12),
        TextField(controller: editNameController, decoration: InputDecoration(labelText: isTeam ? '팀원 명단' : '이름(부수)')),
      ]),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소')),
        TextButton(
          onPressed: () {
            _secureSettingUpdate(() {
              final idx = currentEvent!.players.indexWhere((e) => e.id == player.id);
              if (idx >= 0) {
                currentEvent!.players[idx] = Player(id: player.id, name: editNameController.text.trim(), affiliation: editAffController.text.trim());
              }
              saveData();
              Navigator.pop(ctx);
            }, isPlayerAction: true);
  ...
  void _addPlayer() {
    // 본선 토너먼트가 시작되었는지 확인
    if (currentEvent!.knockoutRounds != null && currentEvent!.knockoutRounds!.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('본선 토너먼트가 시작되어 선수를 추가할 수 없습니다.'), backgroundColor: Colors.redAccent)
      );
      return;
    }
    ...
    _secureSettingUpdate(() {
      ...
      if (isTeam) {
        ...
        // 팀명 중복 체크 (수정 모드일 때는 현재 편집 중인 팀 제외)
        if (_editingTeamId != null) {
          // 수정 모드: 현재 편집 중인 팀의 팀명이 변경되었는지 확인
          final existingTeam = currentEvent!.players.firstWhere(
            (e) => e.id == _editingTeamId,
            orElse: () => Player(id: '', name: '', affiliation: '')
          );
          // 팀명이 변경되었고, 다른 팀과 중복되는지 확인
          if (existingTeam.affiliation != teamName) {
            final hasDuplicate = currentEvent!.players.any(
              (p) => p.id != _editingTeamId && p.affiliation.trim() == teamName
            );
            if (hasDuplicate) {
              ScaffoldMessenger.of(context).showSnackBar(
        ...
        for (int i = 0; i < totalMembers && i < _teamMemberControllers.length; i++) {
          if (_teamMemberControllers[i].text.isNotEmpty) {
            ...
            // 중복 체크: 다른 팀에 이미 등록된 선수인지 확인
            if (!isInCurrentEditingTeam && _isPlayerInRegisteredTeams(playerName)) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('[$playerName] 선수는 이미 다른 팀에 등록되어 있습니다.'), duration: const Duration(seconds: 2))
              );
              return;
            }
        ...
        if (_editingTeamId != null) {
          ...
          if (idx >= 0) {
            // 기존 팀의 선수들을 제외하고 중복 체크
            final existingTeam = currentEvent!.players[idx];
            final existingMembers = existingTeam.name.split(', ').map((s) => s.trim()).toList();
            
            // 새로 추가되는 선수들만 중복 체크
            for (var newName in names) {
              if (!existingMembers.contains(newName) && _isPlayerInRegisteredTeams(newName)) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('[$newName] 선수는 이미 다른 팀에 등록되어 있습니다.'), duration: const Duration(seconds: 2))
                );
                return;
              }
            }
            
            currentEvent!.players[idx] = Player(
              id: _editingTeamId!,
              name: names.join(', '),
              affiliation: _teamNameController.text.trim()
            );
          }
          _editingTeamId = null; // 수정 모드 해제
        } else {
          // 새로 추가
        ...
        currentEvent!.players.insert(0, Player(
          ...
          affiliation: _affController.text.isEmpty ? '개인' : _affController.text.trim()
        ));
  ...
  void _deleteEventDialog(int index) {
    ...
    if (event.players.isNotEmpty) {
      final pwController = TextEditingController();
      showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text('종목 삭제 (주의)'), content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('[${event.name}] 종목을 삭제하시겠습니까?'), const SizedBox(height: 8), const Text('※ 등록된 선수 명단이 함께 삭제됩니다.', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)), const SizedBox(height: 16), TextField(controller: pwController, obscureText: true, decoration: const InputDecoration(labelText: '관리자 비밀번호 입력', border: OutlineInputBorder()))],), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소')), TextButton(onPressed: () { if (pwController.text == '1234') { setState(() { events.removeAt(index); selectedEventIdx = 0; }); saveData(); Navigator.pop(ctx); } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('비밀번호가 틀렸습니다.'))); } }, child: const Text('삭제', style: TextStyle(color: Colors.red)))]));
    } else {
