...
class TournamentLogic {
  static List<Group> generateGroups(List<Player> players, TournamentSettings settings) {
    ...
    for (int i = 0; i < groupCount; i++) {
      ...
      groups.add(Group(
        ...
        matches: generateRoundRobin(groupPlayers, i + 1),
      ));
    }
    return groups;
  }
  ...
  static List<Match> generateRoundRobin(List<Player> players, int groupIdx) {
    ...
    // 탁구 대회 표준 경기 순서 (3인: 1-3, 2-3, 1-2 / 4인: 1-4, 2-3, 1-3, 2-4, 1-2, 3-4)
    List<List<int>> order = [];
    if (n == 3) {
      order = [[0, 2], [1, 2], [0, 1]];
    } else if (n == 4) {
      order = [[0, 3], [1, 2], [0, 2], [1, 3], [0, 1], [2, 3]];
    } else if (n == 5) {
      order = [[0, 4], [1, 3], [0, 2], [4, 3], [1, 2], [0, 3], [4, 2], [1, 0], [3, 2], [4, 1]];
    } else {
      // 그 외 인원은 기본 순환 방식
      for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
  ...
  static Map<Player, Map<String, int>> getRankingStats(Group group) {
    Map<Player, Map<String, int>> stats = {
      for (var p in group.players) p: {'wins': 0, 'won': 0, 'lost': 0, 'diff': 0, 'withdrawn': 0}
    };
    ...
    for (var p in group.players) {
      stats[p]!['diff'] = stats[p]!['won']! - stats[p]!['lost']!;
    }
    return stats;
  }
  ...
  static List<Player> getGroupRankings(Group group) {
    ...
    ranked.sort((a, b) {
      if (stats[a]!['withdrawn'] != stats[b]!['withdrawn']) {
        return stats[a]!['withdrawn']!.compareTo(stats[b]!['withdrawn']!);
      }
  ...
  static List<Round> generateKnockout(List<Player> qualifiedPlayers) {
    ...
    for (int i = 0; i < qualifiedPlayers.length; i++) {
      seededPlayers[seedOrder[i] - 1] = qualifiedPlayers[i];
    }
    List<Round> rounds = [];
    List<Match> firstMatches = [];
    for (int i = 0; i < bracketSize ~/ 2; i++) {
      Player? p1 = seededPlayers[i * 2];
      Player? p2 = seededPlayers[i * 2 + 1];
  ...
  static List<int> _getSeedOrder(int size) {
    ...
    while (currentSize < size) {
      List<int> nextSeeds = [];
      int nextSum = currentSize * 2 + 1;
      for (int s in seeds) {
        nextSeeds.add(s);
        nextSeeds.add(nextSum - s);
      }
      seeds = nextSeeds;
      currentSize *= 2;
    }
