...
class _KnockoutPageState extends State<KnockoutPage> {
  ...
  Widget _buildMatchCard(Match m, int roundIndex, int matchIndex) {
    ...
    // [단체전 전용 로직] 대진 상대가 확정되지 않은 경우 인쇄 아이콘을 숨김
    if (_isTeamMatch && (m.player1 == null || m.player2 == null)) {
      showIcon = false;
    }
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(p?.affiliation ?? (p == null ? 'BYE' : 'TBD'), 
                        style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isW ? const Color(0xFF1A535C) : Colors.black), 
                        overflow: TextOverflow.ellipsis),
                    if (p != null) 
                      Text(_formatTeamMembers(p.name), 
                          style: TextStyle(
                            fontSize: 12, 
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: isW ? const Color(0xFF1A535C) : Colors.grey.shade200,
                  borderRadius: BorderRadius.circular(4),
                ),
                child: Text(withdrawn ? '기권' : '$s', 
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: isW ? Colors.white : Colors.black)),
              ),
            ],
          ),
        ),
  ...
  Widget _playerPopupInfo(Player? p, {required TextAlign textAlign}) => Column(
          ...
          children: [
            Text(_isTeamMatch ? (p?.affiliation ?? "TBD") : (p?.name ?? "TBD"), 
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold), 
                overflow: TextOverflow.ellipsis),
            Text(_isTeamMatch ? (p?.name ?? "") : (p?.affiliation ?? ""), 
